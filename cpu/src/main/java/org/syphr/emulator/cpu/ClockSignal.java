...
{
    void await(long cycle) throws InterruptedException;

    record Frequency(Duration duration) implements ClockSignal
    {
        ...
        {
            return new Frequency(this.duration.dividedBy(multiplier));
        }

        public Frequency dividedBy(int divisor)
        {
            return new Frequency(this.duration.multipliedBy(divisor));
        }
